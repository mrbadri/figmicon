...
export const generatorCommand = async (options: GeneratorOptions = {}) => {
  try {
    ...
    if (!generator) {
      console.log(
        generatorLogger(),
        lightYellow("⚠️  No SVGR configuration found in config file")
      );
      console.log(
        generatorLogger(),
        "Add 'generator' configuration to your icon.config.ts file"
      );
      return;
    }
    ...
    const outputDir =
      options.outputDir || generator.outDir || "src/components/icons";
    ...
    console.log(
      generatorLogger(),
      `📁 Input directory: ${lightGreen(inputDir)}`
    );
    console.log(
      generatorLogger(),
      `📁 Output directory: ${lightGreen(outputDir)}`
    );
    ...
    if (!existsSync(inputDir)) {
      console.log(
        generatorLogger(),
        lightRed(`❌ Input directory '${inputDir}' does not exist`)
      );
      console.log(
        generatorLogger(),
    ...
    if (svgFiles.length === 0) {
      console.log(
        generatorLogger(),
        lightYellow(`⚠️ No SVG files found in ${inputDir}`)
      );
      return;
    }
    ...
    console.log(
      generatorLogger(),
      `🎯 Found ${svgFiles.length} SVG file(s) to process`
    );
    ...
    for (const file of svgFiles) {
      try {
        ...
        const componentName = await generateComponentName(
          file,
          generator.filenameCase
        );

        console.log(
          generatorLogger(),
          `⚙️  Processing: ${file} -> ${componentName}.tsx`
        );
    ...
    console.log(
      generatorLogger(),
      lightGreen(
        `✅ Successfully generated ${generatedFiles.length} React component(s)!`
      )
    );
    console.log(
      generatorLogger(),
      `📦 Components are available in: ${outputDir}`
    );
  } catch (error) {
    console.log(
      generatorLogger(),
...
async function generateComponentName(
  ...
  // Dynamic import of change-case (ESM module)
  const { pascalCase, camelCase, kebabCase, snakeCase } = await import(
    "change-case"
  );
  ...
  switch (caseType) {
    case "camel":
      ...
      break;
    case "kebab":
      componentName = kebabCase(name);
      break;
    case "snake":
      componentName = snakeCase(name);
      break;
    case "pascal":
    default:
  ...
  if (caseType === "pascal" || caseType === "camel") {
    ...
    if (/^[0-9]/.test(componentName)) {
      componentName =
        caseType === "pascal" ? `Svg${componentName}` : `svg${componentName}`;
    }
