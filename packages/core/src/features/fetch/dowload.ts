...
import { formatName, FormatNameCase } from '@/utils/name-format';
import { type Node } from '@figma/rest-api-spec';
import { blue, bold, cyan, gray, green, lightRed, lightYellow, red } from 'kolorist';
...
export const downloadNode = async ({
  ...
  if (!res.ok) {
    ...
    console.error(
      ...
      blue('please wait a moment and try again or check your internet connection.')
    );
    throw new Error(`Failed to call Figma API: ${res.status} ${res.statusText}`);
  }

  const json: { images: Record<string, string>; err?: string } = await res.json();
  ...
  if (!svgRes.ok) {
    throw new Error(`Failed to download SVG: ${svgRes.status} ${svgRes.statusText}`);
  }

  const svg = await svgRes.text();

  let relPath = formatName(node.name || node.id, filenameCase);

  // if (sanitizeName) {
  //   relPath = `${sanitizeFileName(formatName(node, parentNode), node.id)}.svg`;
  // } else {
  //   relPath = `${formatName(node, parentNode)}.svg`;
  // }
